...
import { 
    ...
    Terminal, Bell, FolderKanban, MessageSquare, Plus, Cpu, Settings, PieChart
} from 'lucide-react';
...
const FuturisticDashboard: React.FC<{ 
    ...
    transactions?: any[]; 
    isEmbedded?: boolean; 
}> = ({ onClose, summary, transactions, isEmbedded }) => {
    const [showModal, setShowModal] = React.useState(false);

    return (
        <div className="fixed inset-4 z-[200] bg-[#05070B]/90 text-[#C7D0E0] font-mono overflow-hidden flex border border-[#1C2A3F] rounded-2xl shadow-[0_0_40px_rgba(0,217,255,0.15)] backdrop-blur-xl">
            {showModal && <SmartReportModal onClose={() => setShowModal(false)} />}
            {/* Left Sidebar */}
            <aside className="w-64 border-r border-[#1C2A3F] bg-[#0B1220]/60 flex flex-col p-6 gap-8">
                ...
                <nav className="flex flex-col gap-2 flex-1">
                    {[
                        ...
                        { icon: PieChart, label: 'Relatório Inteligente', onClick: () => setShowModal(true) },
                        ...
                        <button key={i} onClick={item.onClick} className="flex items-center gap-4 p-3 rounded-lg transition-all hover:bg-[#1C2A3F]/50 hover:text-[#00D9FF] hover:border-l-2 hover:border-[#00D9FF] border-l-2 border-transparent">
                            <item.icon size={18} />
